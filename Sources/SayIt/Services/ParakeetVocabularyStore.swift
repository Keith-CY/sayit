...
final class ParakeetVocabularyStore {
    ...
    private let appSupportFolder = AppIdentity.appSupportFolder
    private let legacyAppSupportFolder = AppIdentity.legacyAppSupportFolder
    ...
    func vocabularyFileURL() throws -> URL {
        ...
        let legacyDirectory = base.appendingPathComponent(self.legacyAppSupportFolder, isDirectory: true)

        self.migrateLegacyVocabularyIfNeeded(legacyDirectory: legacyDirectory, destinationDirectory: directory)
    ...
    private func migrateLegacyVocabularyIfNeeded(legacyDirectory: URL, destinationDirectory: URL) {
        let legacyFile = legacyDirectory.appendingPathComponent(self.fileName)
        let destinationFile = destinationDirectory.appendingPathComponent(self.fileName)
        ...
        do {
            ...
            try FileManager.default.copyItem(at: legacyFile, to: destinationFile)
            DebugLogger.shared.debug(
                "ParakeetVocabularyStore: migrated legacy vocabulary file from \(legacyFile.path) to \(destinationFile.path)",
                source: "ParakeetVocabularyStore"
            )
        } catch {
            DebugLogger.shared.warning(
                "ParakeetVocabularyStore: failed to migrate legacy vocabulary file: \(error.localizedDescription)",
                source: "ParakeetVocabularyStore"
            )
        }
    }
    ...
    static func defaultTemplateJSON() -> String {
        let aliases = AppIdentity.defaultDictionaryAliases
            .map { "\"\($0)\"" }
            .joined(separator: ", ")
        ...
        return """
        {
            ...
            "terms": [
                {
                    "text": "\(AppIdentity.defaultDictionaryTerm)",
                    "aliases": [\(aliases)],
                    "weight": 10.0
                }
            ]
