...
final class TypingService {
    // Logging toggle (off by default). Enable by setting env SAYIT_TYPING_LOGS=1
    // or UserDefaults bool for key "enableTypingLogs".
    private static var isLoggingEnabled: Bool {
        if let env = ProcessInfo.processInfo.environment["SAYIT_TYPING_LOGS"], env == "1" { return true }
    ...
    /// Best-effort: activates the app with the given PID, unless it's SayIt itself.
    @discardableResult
