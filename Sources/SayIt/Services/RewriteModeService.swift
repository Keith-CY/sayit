...
final class RewriteModeService: ObservableObject {
    ...
    private var diagnosticsEnabled: Bool {
        if ProcessInfo.processInfo.environment["SAYIT_REWRITE_DIAGNOSTICS"] == "1" {
            return true
        }
    ...
    private func callLLM(messages: [Message], isWriteMode: Bool) async throws -> String {
        ...
        // Edit Text mode intentionally disables streaming to reduce CPU/battery churn.
        ...
        DebugLogger.shared.info(
            "Using LLMClient for Edit mode (streaming=\(config.streaming))",
            source: "RewriteModeService"
        )

        // Streaming is disabled in edit mode, keep buffers reset for deterministic results.
        self.rewrittenText = ""
        self.streamingThinkingText = ""
        self.thinkingBuffer.removeAll()
