...
final class ASRService: ObservableObject {
    ...
    /// Heuristic to ensure Chinese speech is routed to a model that can transcribe it reliably.
    ...
    private static var isChineseLocale: Bool {
        ...
        return localeIds.contains {
            ...
            return chineseLocalePrefixes.contains { lower.hasPrefix($0) }
        }
    }
    ...
    private var transcriptionProvider: TranscriptionProvider {
        let model = self.activeSpeechModel
    ...
    var activeProviderName: String {
        self.activeSpeechModel.displayName
    }
    ...
    func resetTranscriptionProvider() {
        let newModel = self.activeSpeechModel
    ...
    private func handleParakeetVocabularyDidChange() {
        let model = self.activeSpeechModel
        guard model == .parakeetTDT else { return }
